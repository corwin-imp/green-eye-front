...
function* handleLogin(action: AuthLoginAction): Iterable<any> {
    ...
    try {
        ...
        //yield put(push(Router.generate(routes.DASHBOARD)));
        yield put(push(Router.generate(routes.AGENCY_INFO, { type: 'info' })));
    } catch (error) {
        yield put({
...
function* handleCompleteReg(action: AuthCompleteRegAction): Iterable<any> {
    const {
        type,
        payload: { data, callbacks },
        payload,
    ...
    try {
        const result: any = yield AuthApi.completeRegistration(data.plainPassword, data.confirm, data.key)
            .catch(({ response }) => {
                return response;
            })
            .then((response) => {
...
export default function* () {
    ...
    yield takeLatest(AuthActions.AUTH_COMPLETE_REGISTRATION, handleCompleteReg);
}
